...
class FileAdapter(
    ...
    override fun onBindViewHolder(holder: FileViewHolder, position: Int) {
        ...
        // Indentación
        val indentPx =
            (item.depth * 14 * ctx.resources.displayMetrics.density).toInt()
        ...
        if (item.isDirectory) {
            ...
            holder.arrow.setColorFilter(
                ContextCompat.getColor(
                    ctx,
                    R.color.explorer_icon
                )
            )
            ...
            val (iconRes, colorRes) =
                getFileIconAndColor(item.extension)
    ...
    private fun getFileIconAndColor(
        extension: String
    ): Pair<Int, Int> {
        ...
        return when (extension.lowercase()) {
            ...
            "js", "jsx" ->
                Pair(R.drawable.ic_file_js, R.color.icon_js)

            "ts", "tsx" ->
                Pair(R.drawable.ic_file_ts, R.color.icon_ts)

            "py" ->
                Pair(R.drawable.ic_file_py, R.color.icon_py)

            "kt", "kts" ->
                Pair(R.drawable.ic_file_kt, R.color.icon_kt)

            "java" ->
                Pair(R.drawable.ic_file_java, R.color.icon_java)

            "html", "htm" ->
                Pair(R.drawable.ic_file_html, R.color.icon_html)

            "css", "scss" ->
                Pair(R.drawable.ic_file_css, R.color.icon_css)

            "json" ->
                Pair(R.drawable.ic_file_json, R.color.icon_json)

            "xml" ->
                Pair(R.drawable.ic_file_xml, R.color.icon_xml)

            "md" ->
                Pair(R.drawable.ic_file_md, R.color.icon_md)

            "sh", "bash" ->
                Pair(R.drawable.ic_file_sh, R.color.icon_sh)
